...
export function ChatInput({
    ...
    return (
        <div className="w-full max-w-3xl mx-auto px-0 sm:px-4">
            ...
            {files.length > 0 && (
                <div className="flex gap-2 mb-2 overflow-x-auto pb-2 px-1">
                    {files.map((file, index) => (
                        <div
                            key={index}
                            className="relative flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-secondary rounded-lg border border-border shrink-0"
                        >
                            ...
                            <span className="text-xs sm:text-sm text-foreground max-w-20 sm:max-w-32 truncate">{file.name}</span>
                            {file.extractedText && (
            ...
            <div
                className={cn(
                    "relative rounded-xl sm:rounded-2xl bg-secondary border transition-all duration-200",
            ...
            >
                ...
                <Textarea
                    ...
                    className="min-h-[44px] sm:min-h-[52px] max-h-[150px] sm:max-h-[200px] resize-none border-0 bg-transparent px-3 sm:px-4 py-2.5 sm:py-3.5 pr-20 sm:pr-32 focus-visible:ring-0 placeholder:text-muted-foreground text-foreground text-sm sm:text-base"
                    disabled={isLoading}
                />

                {/* Drag overlay */}
                {isDragging && (
                    <div className="absolute inset-0 flex items-center justify-center bg-blue-500/10 rounded-xl sm:rounded-2xl border-2 border-dashed border-blue-500">
                        <p className="text-blue-400 font-medium text-sm sm:text-base">Drop files here</p>
                    </div>
                )}
                ...
                <div className="flex items-center justify-between px-2 sm:px-3 pb-2 sm:pb-3">
                    <div className="flex items-center gap-0.5 sm:gap-1">
                        ...
                        <Button
                            ...
                            size="sm"
                            className={cn(
                                "h-7 sm:h-8 gap-1 sm:gap-1.5 px-1.5 sm:px-2",
                                webSearchEnabled
                    ...
                    {isStreaming ? (
                        <Button
                            type="button"
                            size="icon"
                            className="h-7 w-7 sm:h-8 sm:w-8 rounded-lg bg-muted hover:bg-muted/80"
                            onClick={onStop}
                        >
                        ...
                        <Button
                            type="button"
                            size="icon"
                            className="h-7 w-7 sm:h-8 sm:w-8 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50"
